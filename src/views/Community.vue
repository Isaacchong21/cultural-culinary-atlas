        ...
        >
          <v-card-item class="pa-3 border-b-light">
            <div class="d-flex align-center gap-2">
              ...
              <v-menu v-if="isMyPost(post)" location="bottom end" offset="4">
                ...
                <v-list density="compact" nav min-width="120">
                  <v-list-item prepend-icon="mdi-pencil" title="Edit" @click="editPost(post)" />
                  <v-list-item prepend-icon="mdi-delete" title="Delete" base-color="error" @click="deletePost(post._id)" />
                </v-list>
              </v-menu>
            </div>
          </v-card-item>
    ...
    >
      <v-card class="rounded-2xl post-detail-dialog">
        <v-card-item class="pa-3 d-flex justify-space-between align-center border-b-light">
          <div class="d-flex align-center gap-2">
            <v-btn icon="mdi-arrow-left" variant="text" size="small" @click="imagePreview = false" />
          ...
          <v-container class="py-4">
            <div v-if="previewPost" class="d-flex align-center justify-space-between mb-4">
              <div class="d-flex align-center gap-3">
                ...
                <div>
                  <div class="font-weight-bold cursor-pointer hover-orange" @click="goToProfile(previewPost.authorId)">
                    {{ previewPost.author }}
                  </div>
            ...
            <div v-if="previewPost" class="mb-4">
              <h3 v-if="previewPost.title" class="text-h6 font-weight-bold mb-2">{{ previewPost.title }}</h3>
              ...
              <div v-if="previewPost.tags?.length" class="d-flex flex-wrap gap-2 mt-3">
                <v-chip v-for="tag in previewPost.tags" :key="tag" size="small" color="orange" variant="tonal" class="font-weight-medium">
                  #{{ tag }}
                </v-chip>
              </div>
            </div>
            ...
            <div v-if="previewImages.length > 0" class="media-preview-section mb-4">
              <div v-if="previewPost?.videoUrl" class="video-preview-wrapper">
                <video :src="previewPost.videoUrl" class="preview-media" controls autoplay />
              </div>
            ...
            <div v-if="previewPost" class="d-flex align-center justify-space-between mb-4">
              <div class="d-flex align-center gap-4">
                <v-btn variant="text" :color="previewPost.liked ? 'error' : 'grey'" @click="toggleLike(previewPost)" class="text-none">
                  <v-icon :icon="previewPost.liked ? 'mdi-heart' : 'mdi-heart-outline'" size="24" />
                ...
                <v-btn variant="text" :color="previewPost.saved ? 'orange' : 'grey'" @click="toggleSave(previewPost)" class="text-none">
                  <v-icon :icon="previewPost.saved ? 'mdi-bookmark' : 'mdi-bookmark-outline'" size="24" />
                </v-btn>
              </div>
              <v-btn variant="text" color="grey" @click="sharePost(previewPost)" class="text-none">
                <v-icon icon="mdi-share-variant" size="24" />
              </v-btn>
            </div>

            <div v-if="previewPost" class="mb-4">
              <h4 class="text-subtitle-1 font-weight-bold mb-3">Comments ({{ previewPost.commentsList?.length || 0 }})</h4>
              <div class="d-flex align-start gap-3 mb-4">
                <v-avatar size="36" class="border-thin">
                  <v-img :src="getSafeAvatar(userAvatar)" cover />
                </v-avatar>
                <div class="flex-grow-1">
                  <v-textarea v-model="newCommentText" placeholder="Write a comment... (try #foodie)" variant="outlined" density="comfortable" rows="2" auto-grow class="comment-input" rounded="xl" hide-details />
                  <div class="d-flex justify-end mt-2">
                    <v-btn size="small" variant="flat" color="orange" :disabled="!newCommentText.trim() || submittingComment" @click="submitCommentFromDialog" class="text-none">
              ...
              <div v-if="previewPost.commentsList?.length" class="comments-list">
                <div v-for="(comment, i) in previewPost.commentsList" :key="i" class="mb-3">
    ...
    <v-dialog v-model="commentDialog" max-width="600" persistent scrollable>
      <v-card class="rounded-2xl">
          ...
          <div v-else v-for="(comment, i) in currentPost?.commentsList" :key="i" class="mb-4">
            <div class="d-flex align-start gap-3">
              ...
              <div class="flex-grow-1">
                ...
                <v-btn size="x-small" variant="text" color="grey-darken-1" class="ml-9 mt-1 text-none" @click="replyToComment(comment)">Reply</v-btn>
              </div>
            </div>
          </div>
        </v-card-text>
        <v-card-actions class="pa-4 border-t bg-grey-lighten-5">
          ...
          <v-text-field v-model="currentPost.newComment" placeholder="Write a comment... (try #foodie)" density="compact" variant="outlined" rounded="pill" hide-details @keyup.enter="submitComment">
            <template #append-inner>
              <v-btn size="small" variant="flat" color="orange" :disabled="!currentPost?.newComment?.trim()" @click="submitComment" class="text-none">Post</v-btn>
            </template>
          </v-text-field>
        </v-card-actions>
      </v-card>
...
const moodEmojis = { 'delicious': '😋', 'amazing': '😍', 'excited': '🤩', 'cool': '😎', 'celebrating': '🥳' }

function getAuthorId(authorId) {
  if (!authorId) return ''
  if (typeof authorId === 'string') return authorId
  if (typeof authorId === 'object' && authorId !== null) return authorId._id || String(authorId)
...
const displayedPosts = computed(() => {
  ...
  if (activeFilter.value === 'images') list = list.filter(p => hasMedia(p) && !p.videoUrl)
  else if (activeFilter.value === 'videos') list = list.filter(p => p.videoUrl)
  else if (activeFilter.value === 'popular') list = list.filter(p => p.likes >= 10)

  if (sortBy.value === 'likes') list.sort((a, b) => b.likes - a.likes)
  else if (sortBy.value === 'comments') list.sort((a, b) => (b.comments || 0) - (a.comments || 0))
  else list.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
  
  return list
})

function hasMedia(post) { return post.videoUrl || post.image || post.images?.length > 0 }
function getImageUrl(img) { return typeof img === 'string' ? img : img.url }
function getPostImages(post) {
  if (post.images?.length) return post.images.sort((a, b) => (a.order || 0) - (b.order || 0))
  if (post.image) return [{ url: post.image }]
  return []
}
function getFirstImage(post) { const images = getPostImages(post); return images[0]?.url || images[0] || '' }
function getMediaCount(post) { if (post.videoUrl) return 1; return getPostImages(post).length }
function getPostThumb(url) { if (!url) return ''; if (url.includes('unsplash.com')) return url + '?w=400&q=80'; return url }
function formatDuration(seconds) { if (!seconds) return ''; return `${Math.floor(seconds / 60)}:${Math.floor(seconds % 60).toString().padStart(2, '0')}` }

function toggleVideoPlay(postId) {
  const video = document.querySelector(`video[data-post-id="${postId}"]`)
  if (!video) return
  if (video.paused) {
    document.querySelectorAll('video').forEach(v => { if (v !== video && v.dataset.postId) { v.pause(); isPlayingMap.value[v.dataset.postId] = false } })
    video.play()
  } else { video.pause() }
}
...
function handlePreviewKeydown(e) {
  ...
  if (e.key === 'ArrowRight' && previewImages.value.length > 1) previewIndex.value = (previewIndex.value + 1) % previewImages.value.length
  if (e.key === 'ArrowLeft' && previewImages.value.length > 1) previewIndex.value = (previewIndex.value - 1 + previewImages.value.length) % previewImages.value.length
...
async function submitCommentFromDialog() {
  ...
  try {
    const res = await fetch(`/api/posts/${previewPost.value._id}/comment`, {
      ...
      body: JSON.stringify({ author: localStorage.getItem('userName') || 'Foodie', text: newCommentText.value, avatar: userAvatar.value })
    })
    ...
    if (index > -1) { posts.value[index].commentsList = updatedPost.commentsList; posts.value[index].comments = updatedPost.comments }
    newCommentText.value = ''
...
function sharePost(post) {
  ...
  else copyPostLink(post)
}

async function toggleLike(post) {
  const originalLiked = post.liked; const originalLikes = post.likes
  post.justLiked = true; post.liked = !post.liked; post.likes += post.liked ? 1 : -1
  try {
    const currentUserId = localStorage.getItem('userId'); const currentUserName = localStorage.getItem('userName')
    const res = await fetch(`/api/posts/${post._id}/like`, {
      method: "PUT", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ liked: post.liked, likerId: currentUserId, likerName: currentUserName })
    })
    if (!res.ok) throw new Error("Failed to like")
    const updatedPost = await res.json()
    post.liked = updatedPost.liked; post.likes = updatedPost.likes
    if (updatedPost.likedBy) post.likedBy = updatedPost.likedBy
    setTimeout(() => { post.justLiked = false }, 400)
  } catch (err) { post.liked = originalLiked; post.likes = originalLikes; console.error('Failed to like:', err) }
}

async function toggleSave(post) {
  const originalState = post.saved; post.saved = !post.saved
  try {
    const method = post.saved ? 'POST' : 'DELETE'
    const res = await fetch(`/api/posts/${post._id}/save`, {
      method: method, headers: { 'Content-Type': 'application/json', 'X-User-Id': localStorage.getItem('userId') }
    })
    if (!res.ok) { post.saved = originalState; throw new Error('Failed to save post') }
  } catch (err) { console.error('Failed to toggle save:', err); alert('Failed to save post.'); post.saved = originalState } 
}

function openCommentDialog(post) { currentPost.value = { ...post, newComment: '' }; commentDialog.value = true }
function replyToComment(comment) { currentPost.value.newComment = `@${comment.author} ` }
...
async function submitComment() {
  ...
  try {
    const res = await fetch(`/api/posts/${currentPost.value._id}/comment`, {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ author: localStorage.getItem('userName') || 'Foodie', text: currentPost.value.newComment, avatar: userAvatar.value })
    })
    if (!res.ok) throw new Error("Failed to add comment")
    const updatedPost = await res.json()
...
async function loadPosts(pageNum = 1) {
  ...
  try {
    ...
    const postsWithStatus = (data.posts || data).map(post => {
      let authorIdStr = post.authorId; let authorAvatar = post.avatar
      ...
      return { ...post, authorId: authorIdStr, authorAvatar: authorAvatar, 
    ...
    if (pageNum === 1) posts.value = postsWithStatus
    else posts.value = [...posts.value, ...postsWithStatus]
...
async function refreshPosts() {
  refreshing.value = true; page.value = 1; await loadPosts(1); refreshing.value = false
}

async function loadMorePosts() {
  if (loadingMore.value || !hasMore.value) return
  loadingMore.value = true; page.value++; await loadPosts(page.value); loadingMore.value = false
}

function handleScroll() {
  const scrollTop = window.scrollY + window.innerHeight; const docHeight = document.documentElement.scrollHeight
  if (scrollTop >= docHeight - 500 && hasMore.value && !loadingMore.value) loadMorePosts()
}

function isMyPost(post) {
  const currentUserId = localStorage.getItem('userId')
  if (!currentUserId) return false
  return getAuthorId(post.authorId) === currentUserId
}

function editPost(post) {
  router.push({ path: '/post', query: { edit: post._id } })
}

async function deletePost(postId) {
  if (!confirm('Are you sure you want to delete this post? This cannot be undone.')) return
  try {
    const res = await fetch(`/api/posts/${postId}`, { method: 'DELETE', headers: { 'X-User-Id': localStorage.getItem('userId') } })
  ...
  } catch (err) { console.error('Delete error:', err); alert('Failed to delete post: ' + err.message) }
}

onMounted(async () => {
  const freshAvatar = localStorage.getItem('userAvatar')
  if (freshAvatar && freshAvatar !== 'null' && freshAvatar.trim()) userAvatar.value = freshAvatar
  await loadPosts(1)
...
onActivated(() => {
  if (localStorage.getItem('communityNeedsRefresh') === 'true') {
    localStorage.removeItem('communityNeedsRefresh')
    console.log('🔄 Refreshing community posts after edit/create...')
    refreshPosts()
...
.action-btn { min-width: auto !important; padding: 0 6px !important; border-radius: 6px !important; transition: all 0.2s ease; }
.action-btn:hover { background: rgba(0,0,0,0.04) !important; }
.action-count { font-size: 0.8rem; font-weight: 500; }
.heart-pop { animation: heartPop 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); }
@keyframes heartPop { 0% { 
  transform: scale(1); 
} 
...
@media (prefers-color-scheme: dark) {
  .post-card { background: #1e1e2e; border-color: rgba(255,255,255,0.08); }
  .author-name { color: #e2e8f0; }
  .post-content { color: #cbd5e1; }
  .border-b-light, .border-t-light { border-color: rgba(255,255,255,0.08) !important; }
  .comment-input :deep(.v-field) { background: #2a2a4a !important; border-color: rgba(255,255,255,0.1) !important; }
  .comments-list::-webkit-scrollbar-track { background: #1e1e2e; }
  .comments-list::-webkit-scrollbar-thumb { background: #4a4a6a; }
}
...
@media (max-width: 600px) {
  ...
  .post-content.compact-content { font-size: 0.8rem; -webkit-line-clamp: 2; line-clamp: 2; }
  .post-video, .grid-image { height: 180px !important; }
  .action-count { display: none; }
  .preview-media { height: 300px !important; }
